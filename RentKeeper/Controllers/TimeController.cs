...
namespace RentKeeper.Controllers
{
    ...
    public class TimeController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<TimeDto>> Create(TimeDto dto)
        {
            var time = _mapper.Map<Time>(dto);
            await _service.CreateAsync(time);
            var read = _mapper.Map<TimeDto>(time);
        ...
        public async Task<ActionResult<TimeDto>> GetById(int id)
        {
            ...
            return Ok(_mapper.Map<TimeDto>(t));
        }

        [HttpGet]
        public async Task<ActionResult<IEnumerable<TimeDto>>> GetAll([FromQuery] int page = 1, [FromQuery] int pageSize = 10)
        {
            var list = await _service.GetAllAsync(page, pageSize);
            return Ok(_mapper.Map<IEnumerable<TimeDto>>(list));
        }

        [HttpPut("{id}")]
        public async Task<ActionResult<TimeDto>> Update(int id, TimeDto dto)
        {
            var time = _mapper.Map<Time>(dto);
            var upd = await _service.UpdateAsync(id, time);
            return Ok(_mapper.Map<TimeDto>(upd));
        }
